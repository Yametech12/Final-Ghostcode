...
export function EmptyState({ onSelectPrompt, disabled }: EmptyStateProps) {
  return (
    ...
    >
      <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-accent-primary to-accent-secondary flex items-center justify-center shadow-lg shadow-accent-primary/20 mb-4">
        <Bot aria-hidden="true" className="w-7 h-7 text-white" />
      </div>
        ...
        {promptCategories.map(category => (
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              <Sparkles aria-hidden="true" className="w-3.5 h-3.5 text-accent-primary" />
              ...
              {category.prompts.map(prompt => (
                <li key={prompt}>
                  <button
                    ...
                    className="group/prompt relative w-full text-left text-xs text-slate-200 hover:text-white bg-white/[0.03] hover:bg-accent-primary/10 border border-white/5 hover:border-accent-primary/40 rounded-lg px-3 py-2.5 pr-8 transition-all duration-150 active:scale-[0.98] disabled:opacity-40 disabled:cursor-not-allowed"
                  >
