...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE bucket_id = 'user-uploads' AND name = 'Users can upload their own files'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM storage.policies 
    WHERE bucket_id = 'user-uploads' AND name = 'Users can view their own files'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM storage.policies 
    WHERE bucket_id = 'user-uploads' AND name = 'Public can view files'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM storage.policies 
    WHERE bucket_id = 'user-uploads' AND name = 'Users can delete their own files'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM storage.policies 
    WHERE bucket_id = 'user-uploads' AND name = 'Users can update their own files'
  ) THEN
